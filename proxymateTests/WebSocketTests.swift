...
final class WebSocketTests: XCTestCase {
    ...
    func testParseTextFrame() {
        // Build a simple unmasked text frame: "Hello"
        let payload = Data("Hello".utf8)
        var frame = Data([0x81]) // FIN + text opcode
        frame.append(UInt8(payload.count)) // no mask, length < 126
        frame.append(payload)

        let parsed = WebSocketInspector.parseFrame(frame)
    ...
    func testParseMaskedFrame() {
        let payload = Data("Hi".utf8)
        let mask: [UInt8] = [0x12, 0x34, 0x56, 0x78]
        var masked = payload
        for i in 0..<masked.count { masked[i] ^= mask[i % 4] }

        var frame = Data([0x81]) // FIN + text
        frame.append(0x80 | UInt8(payload.count)) // masked flag + length
    ...
    func testParseCloseFrame() {
        var frame = Data([0x88, 0x02]) // FIN + close, 2 bytes
        frame.append(contentsOf: [0x03, 0xE8]) // code 1000
    ...
    func testParseIncompleteFrame() {
        let frame = Data([0x81]) // just first byte, no length
        XCTAssertNil(WebSocketInspector.parseFrame(frame))
    }

    func testParseBinaryFrame() {
        var frame = Data([0x82, 0x03]) // FIN + binary, 3 bytes
    ...
    func testParse16BitLength() {
        let payload = Data(repeating: 0x41, count: 200)
        var frame = Data([0x81, 126]) // FIN + text, 16-bit length
    ...
    func testInspectBinarySkipped() {
        ...
        let result = WebSocketInspector.inspect(frame: frame, host: "example.com", rules: [rule])
        XCTAssertFalse(result.blocked) // binary frames are not inspected
    }
    ...
    func testBuildCloseFrame() {
        ...
        XCTAssertEqual(frame[0], 0x88) // FIN + close opcode
